...
json_path = f'../temp/jackknife_spectra_treated_snv_50_cotton_nir.json'

loaded_dict = json.load(open(json_path,"r"))
for wn in loaded_dict.keys():
   jackknife_agg = loaded_dict[wn]
   #jackknife.plot_jackknife(jackknife_agg, wn)
...
#jackknife.plot_jackknife(averaged_data, wn="Top 10 wavenumbers")
...
def chi_squared_test(sample_variance, full_variance, sample_size, confidence_level=0.95):
    # Degrees of freedom
    df = sample_size - 1
...
if chi_squared_test(sample_variance=200, full_variance=14, sample_size=200, confidence_level=0.99):
