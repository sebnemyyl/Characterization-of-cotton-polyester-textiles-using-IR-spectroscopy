...
spectra_reproducibility <- spectra_df_full %>% filter(reference.pet == 50 & reference.measuring_date >= 240807)

#spectra_df_clean <- clean_up_spectra(spectra_df_full)
plot_spectra(spectra_reproducibility)
#baseline_correction_df <- als(spectra_reproducibility, 3, 0.05,20)
baseline_correction_df <- stdnormalvariate(spectra_reproducibility)
baseline_csv_path <- file.path(TEMP_DIR, "spectra_treated.csv")
